...
import { Loader2, Bell, CheckCircle2, XCircle, Eye, Check, ArrowRightLeft, ClipboardList, FileText, Send, Mail, Mailbox, ChevronDown, ChevronUp } from "lucide-react";
...
export default function NotificationsPage() {
  ...
  const myPendingSchedules = useMemo(() => schedules.filter(s => 
    s.status === 'pending' && userId && Array.isArray(s.userIds) && s.userIds.map(String).includes(String(userId))
  ), [schedules, userId]);
  ...
  return (
    <div className="space-y-8">
      ...
      {pendingProjectApprovals.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => toggleSection('projects')}>
            ...
            {expandedSections['projects'] ? <ChevronUp className="h-5 w-5 text-slate-500" /> : <ChevronDown className="h-5 w-5 text-slate-500" />}
          </CardHeader>
            ...
            {pendingProjectApprovals.map(project => (
              <div key={project.id} className="bg-white p-4 rounded-lg border shadow-sm flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="flex items-center gap-2 self-end md:self-center">
                  ...
                  <Button variant="outline" size="sm" className="text-slate-600" onClick={() => {
                    // setLocation(`/projects?editProject=${project.id}&tab=itens`)
      ...
      {myPendingSchedules.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => toggleSection('schedules')}>
            <div>
            ...
            {expandedSections['schedules'] ? <ChevronUp className="h-5 w-5 text-slate-500" /> : <ChevronDown className="h-5 w-5 text-slate-500" />}
          </CardHeader>
          {expandedSections['schedules'] && (
            <CardContent className="space-y-2 animate-in fade-in-50">
      ...
      {pendingInventoryApprovals.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => toggleSection('inventory')}>
            <div>
            ...
            {expandedSections['inventory'] ? <ChevronUp className="h-5 w-5 text-slate-500" /> : <ChevronDown className="h-5 w-5 text-slate-500" />}
          </CardHeader>
      ...
      {pendingMovementApprovals.length > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between cursor-pointer" onClick={() => toggleSection('movements')}>
            <div>
            ...
            {expandedSections['movements'] ? <ChevronUp className="h-5 w-5 text-slate-500" /> : <ChevronDown className="h-5 w-5 text-slate-500" />}
          </CardHeader>
